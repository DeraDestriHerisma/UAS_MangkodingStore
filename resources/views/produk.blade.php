    ...
    <div class="container">
      ...
      <div class="flex justify-between items-center mt-5">
        <h3 class="text-5xl font-semibold">Our Best Seller Product</h3>
          <a href="/tambahProduk" class="text-black bg-[white] border border-black py-3 px-5 ">+</a>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-28 gap-x-10 mt-10">
      @foreach ($data as $item)
      <div class="relative  ">
        <img src="{{ asset('fotoproduk/'.$item->foto) }}" style="width: 65%" alt="">
        <div class="mt-3 space-y-4">
          <div class="flex justify-between">
            <p class="font-bold text-md">{{ $item->title }}</p>
          </div>
          <p class="font-bold text-md">Rp. {{ $item->harga }}</p>
          <div class="flex justify-between">
            <p class="text-md text-gray-600">{{ $item->deskripsi_product }}</p>
          </div>
          <div class="text-md">
            <a href="/editProduk/{{ $item->id }}" class="btn btn-info">Edit</a> &nbsp
            <a href="/deleteProduk/{{ $item->id }}" class="btn btn-danger">Delete</a> 
          </div>
        </div>
      </div>
      @endforeach
